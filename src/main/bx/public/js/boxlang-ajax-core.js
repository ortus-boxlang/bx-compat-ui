...
window.BoxLangAjax = window.BoxLangAjax || {
    version: '1.0.0',
    config: {
        cssSrc: '/bx-compat-ui/css',
        scriptSrc: '/bx-compat-ui/js',
...
BoxLangAjax.utils.fetchContent = async function(url, options = {}) {
    ...
    for (let attempt = 1; attempt <= BoxLangAjax.config.retryAttempts; attempt++) {
        try {
            ...
            if (attempt < BoxLangAjax.config.retryAttempts) {
                await new Promise(resolve => 
                    setTimeout(resolve, BoxLangAjax.config.retryDelay * Math.pow(2, attempt - 1))
                );
            }
        }
    }
...
BoxLangAjax.utils.loadIntoContainer = function(containerId, url, options = {}) {
    ...
    const loadingHtml = options.loadingTemplate || '<div class="bx-loading">Loading...</div>';
    ...
    return BoxLangAjax.utils.fetchContent(url, options)
        ...
        .catch(function(error) {
            const errorTemplate = options.errorTemplate || 
                `<div class="bx-source-error">
                    ...
                    <div class="bx-error-retry">
                        <button class="bx-retry-button" onclick="BoxLangAjax.utils.loadIntoContainer('${containerId}', '${url}', ${JSON.stringify(options).replace(/'/g, "\\'")})">
                            Retry
                        </button>
                    </div>
                </div>`;
...
function initBoxLangAjax() {
    ...
    window.addEventListener('unhandledrejection', function(event) {
        if (event.reason && event.reason.message && event.reason.message.includes('BoxLang AJAX')) {
            console.error('Unhandled BoxLang AJAX error:', event.reason);
        }
    });
    
    // Initialize auto-refresh elements
    document.querySelectorAll('[data-refresh-url][data-refresh-interval]').forEach(function(element) {
    ...
    document.querySelectorAll('form[data-ajax-target]').forEach(function(form) {
