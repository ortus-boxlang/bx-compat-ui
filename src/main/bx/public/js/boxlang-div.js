...
(function() {
    ...
    BoxLangAjax.components.div = {
        ...
        autoRefresh: function(divId, interval) {
            ...
            if (!url) {
                console.error('No refresh URL found for auto-refresh div: ' + divId);
                return;
            }
        ...
        loadWithOverlay: function(divId, url, options = {}) {
            ...
            overlay.innerHTML = options.loadingTemplate || '<div class="bx-loading">Loading...</div>';
            ...
            return BoxLangAjax.utils.fetchContent(url, options)
                .then(function(content) {
                    ...
                    const event = new CustomEvent('boxlang-div-loaded', {
                ...
                .catch(function(error) {
                    ...
                    const errorHtml = options.errorTemplate || 
                    ...
                    // Trigger error event
                    const errorEvent = new CustomEvent('boxlang-div-error', {
        ...
        appendContent: function(divId, url, options = {}) {
            ...
            return BoxLangAjax.utils.fetchContent(url, options)
                ...
                .catch(function(error) {
                    ...
                    loadingIndicator.className = 'bx-source-error bx-append-error';
    ...
    function autoLoadDivContent() {
        document.querySelectorAll('.bx-div[data-source]').forEach(function(div) {
            ...
            if (url && div.id) {
                setTimeout(function() {
                    if (loadType === 'overlay') {
                        BoxLangAjax.components.div.loadWithOverlay(div.id, url);
                    } else {
                        BoxLangAjax.utils.loadIntoContainer(div.id, url);
                    }
                }, delay);
    ...
    function setupAutoRefresh() {
        document.querySelectorAll('.bx-div[data-refresh-interval]').forEach(function(div) {
    ...
    function initDivAjax() {
        ...
        document.querySelectorAll('[data-refresh-target]').forEach(function(button) {
            button.addEventListener('click', function() {
                ...
                BoxLangAjax.components.div.refresh(targetId, params)
                    .catch(function(error) {
        ...
        document.querySelectorAll('[data-load-more-target]').forEach(function(button) {
            button.addEventListener('click', function() {
                const targetId = button.dataset.loadMoreTarget;
                const url = button.dataset.loadMoreUrl;
                
                if (targetId && url) {
                    BoxLangAjax.components.div.appendContent(targetId, url)
                        .then(function() {
